...
function recipePrompt(req: RecipesRequest): string {
  ...
  const servings = prefs?.servings
    ? `\nScale every recipe to serve exactly ${prefs.servings} ${prefs.servings === 1 ? "person" : "people"}: write the ingredient quantities in steps[] and the amounts in missingIngredients[] for that headcount.`
    : "";
