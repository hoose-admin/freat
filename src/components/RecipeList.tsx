import { useRef, useState } from "react";
import type { Recipe } from "../lib/types";
import StepText from "./StepText";
import CookMode from "./CookMode";

interface Props {
  recipes: Recipe[];
  /** Indices of the recipes the user has added to the shopping list. */
  selected: Set<number>;
  /** Toggle a recipe in/out of the shopping list (lifted to App). */
  onToggleSelect: (index: number) => void;
  /** Regenerate ONE dish with a quick tweak (lifted to App, which owns the
   *  in-place swap). `tweak` is a preset phrase or the free-text nudge. */
  onRemix: (index: number, tweak: string) => void;
  /** Indices currently being remixed — that card shows a spinner + disables its
   *  own remix controls; siblings stay interactive. */
  remixing: Set<number>;
  /** Jump back to the ingredient editor — wired to the empty-state next step. */
  onEditIngredients?: () => void;
}

// Preset one-tap tweaks. `label`/`icon` are for the chip; `tweak` is the phrase
// sent verbatim to the remix prompt (server/gemini.ts `remixPrompt`).
const PRESETS = [
  { icon: "🌶", label: "Spicier", tweak: "make it spicier" },
  { icon: "🥗", label: "Vegan", tweak: "make it vegan" },
  { icon: "⏱", label: "Faster", tweak: "make it quicker to cook" },
  {
    icon: "🔁",
    label: "Different idea",
    tweak: "give me a completely different idea using the same ingredients",
  },
] as const;

export default function RecipeList({
  recipes,
  selected,
  onToggleSelect,
  onRemix,
  remixing,
  onEditIngredients,
}: Props) {
  // Which recipe (if any) is open in Cook Mode. State lives here — the grid owns
  // the recipe→overlay interaction and the focus restoration (matches TKT-109).
  const [cookIndex, setCookIndex] = useState<number | null>(null);
  // The "Cook this" button that opened the overlay, so focus can return to it.
  const triggerRef = useRef<HTMLButtonElement | null>(null);

  function openCook(idx: number, trigger: HTMLButtonElement) {
    triggerRef.current = trigger;
    setCookIndex(idx);
  }

  function closeCook() {
    setCookIndex(null);
    // Restore focus to the originating control on the next tick (after unmount).
    const trigger = triggerRef.current;
    requestAnimationFrame(() => trigger?.focus());
  }

  if (recipes.length === 0) {
    return (
      <section className="recipes recipes--empty">
        <h2 className="section-title" tabIndex={-1}>Meal ideas</h2>
        <p className="muted">
          No recipes matched those ingredients yet. Tap <strong>Edit ingredients</strong> to
          add a couple more, then try again.
        </p>
        {onEditIngredients && (
          <button className="btn btn--ghost" onClick={onEditIngredients}>
            Edit ingredients
          </button>
        )}
      </section>
    );
  }

  const cooking = cookIndex !== null ? recipes[cookIndex] : null;

  return (
    <section className="recipes">
      <h2 className="section-title" tabIndex={-1}>Meal ideas</h2>
      <div className="recipe-grid">
        {recipes.map((r, idx) => (
          <RecipeCard
            key={`${r.title}-${idx}`}
            recipe={r}
            index={idx}
            selected={selected.has(idx)}
            onToggleSelect={onToggleSelect}
            onCook={openCook}
            onRemix={onRemix}
            remixing={remixing.has(idx)}
          />
        ))}
      </div>

      {cooking && <CookMode recipe={cooking} onClose={closeCook} />}
    </section>
  );
}

interface CardProps {
  recipe: Recipe;
  index: number;
  selected: boolean;
  onToggleSelect: (index: number) => void;
  onCook: (index: number, trigger: HTMLButtonElement) => void;
  onRemix: (index: number, tweak: string) => void;
  remixing: boolean;
}

function RecipeCard({
  recipe: r,
  index: idx,
  selected,
  onToggleSelect,
  onCook,
  onRemix,
  remixing,
}: CardProps) {
  // The card's own free-text nudge. Local — each card steers itself, and a
  // remix swaps only this card's recipe (App owns the in-place swap).
  const [nudge, setNudge] = useState("");

  function submitNudge(e: React.FormEvent) {
    e.preventDefault();
    const tweak = nudge.trim();
    if (!tweak || remixing) return;
    onRemix(idx, tweak);
    setNudge("");
  }

  return (
    <article className={selected ? "recipe-card recipe-card--selected" : "recipe-card"}>
      <header className="recipe-card__head">
        <h3 className="recipe-card__title">{r.title}</h3>
        <div className="recipe-card__meta">
          {r.difficulty && <span className={`tag tag--${r.difficulty}`}>{r.difficulty}</span>}
          {typeof r.timeMinutes === "number" && (
            <span className="tag">⏱ {r.timeMinutes} min</span>
          )}
        </div>
      </header>

      <label className="recipe-card__select">
        <input type="checkbox" checked={selected} onChange={() => onToggleSelect(idx)} />
        Add to shopping list
      </label>

      <p className="recipe-card__desc">{r.description}</p>

      {r.usesIngredients.length > 0 && (
        <p className="recipe-card__uses">
          <strong>Uses:</strong> {r.usesIngredients.join(", ")}
        </p>
      )}

      {r.missingIngredients.length > 0 && (
        <p className="recipe-card__missing">
          <strong>You'll also need:</strong> {r.missingIngredients.join(", ")}
        </p>
      )}

      <details className="recipe-card__steps">
        <summary>Steps</summary>
        <ol>
          {r.steps.map((step, i) => (
            <li key={i}>
              <StepText text={step} />
            </li>
          ))}
        </ol>
      </details>

      {/* Remix — regenerate just this dish with a quick tweak. */}
      <div className="recipe-card__remix" role="group" aria-label="Remix this recipe" aria-busy={remixing}>
        <p className="recipe-card__remix-label">Not quite? Remix it</p>
        <div className="remix-chips">
          {PRESETS.map((p) => (
            <button
              key={p.label}
              type="button"
              className="chip chip--action"
              onClick={() => !remixing && onRemix(idx, p.tweak)}
              disabled={remixing}
            >
              <span aria-hidden="true">{p.icon}</span> {p.label}
            </button>
          ))}
        </div>
        <form className="remix-nudge" onSubmit={submitNudge}>
          <input
            className="input"
            type="text"
            value={nudge}
            onChange={(e) => setNudge(e.target.value)}
            placeholder="or tell me how… (e.g. no dairy)"
            aria-label={`Describe a tweak for ${r.title}`}
            disabled={remixing}
          />
          <button
            type="submit"
            className="btn btn--ghost btn--sm"
            disabled={remixing || nudge.trim().length === 0}
          >
            Remix
          </button>
        </form>
        {remixing && (
          <p className="recipe-card__remix-status" role="status">
            Remixing this dish…
          </p>
        )}
      </div>

      {r.steps.length > 0 && (
        <div className="recipe-card__actions">
          <button
            type="button"
            className="btn btn--primary"
            onClick={(e) => onCook(idx, e.currentTarget)}
          >
            <span aria-hidden="true">👨‍🍳</span> Cook this
          </button>
        </div>
      )}
    </article>
  );
}
